...
def origin_spike(dict_seq):
    """
        Return Spike Protein isolated in Whuan. (EPI_ISL_402123)
    """
    seq_orig = dict_seq['EPI_ISL_402124']
...
def lineages_of_interest():
    ## Valid Lineages
    valid_lineage_FDLs = ['B']  # mettere i lineage che definisco come classe# mettere i lineage che definisco come classe

    valid_lineage_newFDLS = ['B.1.1.63']
...
def retraining_weeks():
    retraining_week = [20, 54]
    retraining_week_false_positive = [20, 54]
...
def write_precision(precision,week):
    ...
    filtered_precision_week_1 = [precision[i] for i in range(len(week)) if week[i] > 0]
    ...
    return [
        ['The median is (METHOD FIND IN LITERATURE) ' + str(median_precision_paper), 'The 25th percentile is (paper) ' + str(q1_prec_paper),
